...
test('Gmail HTML preserves nested semantic lists', () => {
  ...
  assert.match(html, /<ul style="font-family: verdana, sans-serif; font-size: 10pt;">/);
...
test('Gmail HTML uses configured font and size', () => {
  ...
  const html = sanitizer.buildGmailHtmlFromDoc(doc, {
    gmailFontFamily: 'arial, sans-serif',
    gmailFontSize: '12pt'
  });
  assert.match(html, /font-family: arial, sans-serif; font-size: 12pt;/);
});
